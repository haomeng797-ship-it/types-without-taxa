# 14b_fpr_skew.R — continuation of 14_fpr_calibration.R: runs ONLY the skew (Bauer-Curran) scenario
# and appends it to the already-saved gauss checkpoint in out/fpr_calibration.rds.
# Detached, capped cores (NC env), so it survives session teardown and runs cooler.
suppressMessages({library(mclust); library(cluster); library(parallel)})
P2<-Sys.getenv("P2"); if(P2=="")P2<-"."
M<-as.integer(Sys.getenv("M","100")); R<-40L; n<-2000L
...
cat(sprintf("skew scenario: M=%d, NC=%d, start %s\n",M,NC,format(Sys.time(),"%H:%M")))
rows<-mclapply(1:M,function(m){
  ...
  data.frame(scenario="skew",m=m,real_med=real,null975=thr,fired=real>thr,bic_k=kb,gap_k=gp,blrt_k=bl) },mc.cores=NC)
skew<-do.call(rbind,rows)
prev<-readRDS(file.path(P2,"out","fpr_calibration.rds"))
prev<-prev[prev$scenario!="skew",]
out<-rbind(prev,skew)
saveRDS(out,file.path(P2,"out","fpr_calibration.rds"))
