...
abstract: |
  ...
  personality; trait structure has long been argued to be dimensional. What is new is the test, which any
  future claim of N types can be held to.
keywords:
...
adding pieces. So the count isn't telling you about the respondents. The model settled it before anyone
looked at the data.
...
test is not a machine that only ever returns "continuous"; when separated types are present in the
dependence it fires, and fires harder the clearer they are.

It also has a blind spot worth stating plainly. When I instead built well-separated clusters whose
separation was large enough to register in the univariate distributions, the test went quiet: the
